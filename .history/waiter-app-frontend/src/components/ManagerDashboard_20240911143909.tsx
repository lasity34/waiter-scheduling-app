...
const ManagerDashboard: React.FC = () => {
  ...
  const handleShiftUpdate = async (shiftId: number) => {
    const shift = shifts.find(s => s.id === shiftId);
    if (!shift) return;

    const shiftTypes: ('morning' | 'evening' | 'double')[] = ['morning', 'evening', 'double'];
    const currentIndex = shiftTypes.indexOf(shift.shiftType);
    const newShiftType = shiftTypes[(currentIndex + 1) % shiftTypes.length];
    ...
    try {
      await axios.put(`http://localhost:5000/shifts/${shiftId}`, {
      ...
      <ContentContainer>
        ...
        <CalendarContainer>
          <Calendar
            ...
            onSelectEvent={(event: Shift) => handleShiftUpdate(event.id)}
